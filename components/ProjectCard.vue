<template>
  <a :href="`https://modrinth.com/${type}/${id}`" target="_blank" class="project-card card" :aria-label="name"
     role="listitem">
    <div class="columns">
      <div class="icon">
        <div>
      ...
      <div class="card-content">
        <div class="info">
          ...
          <div class="versions">
            <span class="version mod-version" v-if="featuredVersions">{{ featuredVersions[0].version_number }}</span>
            <span class="version game-version" v-if="featuredVersions">{{ $formatVersion(featuredVersions[0].game_versions) }}</span>
          </div>
        </div>
      </div>
    </div>
...
export default {
  ...
  async fetch() {
    try {
      const [project, versions, featuredVersions, dependencies] = (
        await Promise.all([
          this.$axios.get(`https://api.modrinth.com/v2/project/${this.id}`),
          this.$axios.get(`https://api.modrinth.com/v2/project/${this.id}/version`),
          this.$axios.get(`https://api.modrinth.com/v2/project/${this.id}/version?featured=true`),
          this.$axios.get(`https://api.modrinth.com/v2/project/${this.id}/dependencies`),
        ])
      ).map((it) => it.data)
      if (project.project_type !== this.type) {
        return
      }
      if (project.body_url && !project.body) {
        project.body = (await this.$axios.get(project.body_url)).data
      }
...
.project-card {
  ...
  @media screen and (min-width: 1024px) {
    ...
    margin: 5px;
    width: calc(100% / 3 - 2 * var(--spacing-card-bg) - 2 * 5px);
  }
