<template>
  <div class="page">
    ...
    <div v-if="!isFile && loaded">
      <NuxtLink to="../" v-if="paths.length > 0" >../</NuxtLink>
      <br/>
  ...
  methods: {
    async setList() {
      try {
        const [mavenData] = (
          await Promise.all([
            this.$axios.get(`https://maven.awakenedredstone.com/${this.$route.params.pathMatch}`)
          ])
        ).map((it) => it.data)
        ...
        if (!mavenData.type) {
          window.open(`https://maven.awakenedredstone.com/${this.$route.params.pathMatch}`);
